...
from CopyxMusic import config, logger
from CopyxMusic.helpers import Track, utils
...
class YouTube:
    def __init__(self):
        ...
        self.copymusic_key = config.ARTISTBOTS_KEY
        ...
        if self.enable_api:
            ...
            if self.copymusic_key:
                masked_key = self.copymusic_key[:8] + "..." if len(self.copymusic_key) > 8 else "***"
    ...
    def get_cookies(self):
        ...
        if not self.checked:
            cookies_dir = "CopyxMusic/cookies"
        ...
        cookie_file = f"CopyxMusic/cookies/{random.choice(self.cookies)}"
    ...
    async def save_cookies(self, urls: list[str]) -> None:
        ...
        cookies_dir = Path("CopyxMusic/cookies")
    ...
    async def download_via_api(self, link: str, video: bool = False) -> Optional[str]:
        """
        Download audio/video using CopyMusic API (Primary Method).
        ...
        try:
            ...
            logger.info(f"🚀 [API PRIMARY] Trying CopyMusic API for {video_id} (type: {download_type})")
            ...
            if self.copymusic_key:
                params["api_key"] = self.copymusic_key
                logger.debug(f"Using API key: {self.copymusic_key[:8]}...")
            else:
                logger.warning("No CopyMusic API key configured!")
                return None
    ...
    async def download(self, video_id: str, is_live: bool = False, video: bool = False) -> Optional[str]:
        ...
        if self.enable_api and self.api_url and self.copymusic_key:

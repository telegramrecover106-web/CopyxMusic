...
from CopyxMusic import tune, app, config, db, lang, logger, queue, tasks, userbot, yt
...
async def _vc_started(_, m: types.Message):
    """Announce and register a newly started voice/video chat."""
    await db.mark_voice_chat(m.chat.id, True)
    try:
        await app.send_message(m.chat.id, "😍ᴠɪᴅᴇᴏ ᴄʜᴀᴛ sᴛᴀʀᴛᴇᴅ🥳")
    except Exception:
        pass
...
async def _vc_ended(_, m: types.Message):
    """Stop playback and clear queue whenever the group call ends."""
    chat_id=m.chat.id
    await db.mark_voice_chat(chat_id, False)
    try:
        await tune.stop(chat_id)
    except Exception:
        pass
    try:
        await app.send_message(chat_id, "😕ᴠɪᴅᴇᴏ ᴄʜᴀᴛ ᴇɴᴅᴇᴅ💔\n\n<b>Queue and playback records have been cleared.</b>")
    except Exception:
        pass
    ...
    async def update_chat_timer(chat_id):
        ...
        while True:
            try:
                ...
                # Generate progress bar (clean block style)
                bar_length = 9
                if duration == 0:
                    percentage = 0
                else:

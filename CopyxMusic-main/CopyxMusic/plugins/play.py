...
from CopyxMusic import tune, app, config, db, lang, queue, tg, yt
from CopyxMusic.helpers import buttons, utils
from CopyxMusic.helpers._play import checkUB
import asyncio
import logging
...
_PLAY_FRAMES = ["🔎 <b>Searching...</b>", "💃 <b>Finding your song...</b>", "🔎 <b>Searching...</b>"]
...
async def play_hndlr(
    ...
    # Send lightweight animated searching message
    try:
    ...
    if force:
        ...
        if await db.get_call(chat_id):
            ...
            try:
                from CopyxMusic import preload
    ...
    try:
        ...
        error_msg = str(e)
        if any(x in error_msg.lower() for x in ["no active group call", "noactivegroupcall", "groupcall_invalid"]):
            await safe_edit(sent, "📵 No active voice chat found.\n\nPlease start a voice chat and try again.💔")
            try:
                queue.clear(chat_id)
        ...
        return
    try:
        await db.add_history(m.from_user.id, file)
    except Exception:
        pass
    if not tracks:
        return

...
from CopyxMusic import config, logger, userbot


class NoAssistantAvailable(Exception):
    """Raised when no assistant client is currently connected."""
    pass
...
class MongoDB:
    ...
    # FAVOURITES / HISTORY
    async def add_favourite(self, user_id: int, media) -> None:
        doc = {
        ...
        await self.db.favourites.update_one(
            {"_id": user_id}, {"$addToSet": {"items": doc}}, upsert=True
        )

    async def remove_favourite(self, user_id: int, title: str) -> bool:
        result = await self.db.favourites.update_one(
            {"_id": user_id}, {"$pull": {"items": {"title": title}}}
        )
        return result.modified_count > 0

    async def get_favourites(self, user_id: int, limit: int = 20) -> list:
        doc = await self.db.favourites.find_one({"_id": user_id}) or {}
        return (doc.get("items") or [])[:limit]

    async def add_history(self, user_id: int, media) -> None:
        doc = {
            "title": getattr(media, "title", "Unknown"),
            "url": getattr(media, "url", ""),
            "duration": getattr(media, "duration", "0:00"),
        ...
        await self.db.history.update_one(
            {"_id": user_id}, {"$push": {"items": {"$each": [doc], "$slice": -50}}}, upsert=True
        )
    ...
    # CACHE
    async def mark_voice_chat(self, chat_id: int, active: bool) -> None:
        if active: self.voice_chats.add(chat_id)
        else: self.voice_chats.discard(chat_id)

    def voice_chat_known_active(self, chat_id: int) -> bool:
    ...
    async def get_admins(self, chat_id: int, reload: bool = False) -> list[int]:
        from CopyxMusic.helpers._admins import reload_admins
    ...
    async def set_assistant(self, chat_id: int) -> int:
        if not userbot.clients:
            raise NoAssistantAvailable(
                "No assistant clients are currently connected. "
                "Check STRING_SESSION(s) in your .env for AUTH_KEY_DUPLICATED "
                "or invalid session errors."
            )

...
from CopyxMusic import app, config, db, lang, logger, preload, queue, userbot, yt
from CopyxMusic.helpers import Media, Track, buttons, thumb
...
class TgCall(PyTgCalls):
    ...
    async def stop(self, chat_id: int) -> None:
        # Always clear local state first; an unavailable assistant must not leave
        # stale queue/playback records behind.
        client = None
        ...
        try:
            queued_items = queue.get_all(chat_id)
        ...
        try:
            ...
            if not seek_time:
                ...
                text = _lang["play_media"].format(
                    ...
                    media.duration,
                    media.user,
                    getattr(media, "view_count", None) or "N/A",
                    max(0, len(queue.get_queue(chat_id)) - 1),
                )
            ...
            async def update_handler(_, update: types.Update) -> None:
                if isinstance(update, types.StreamEnded):
                    ...
                    if update.status in [
                        ...
                        try:
                            await db.mark_voice_chat(update.chat_id, False)
                        except Exception:
                            pass

...
from CopyxMusic import app, config, db, queue, yt
from CopyxMusic.core.mongo import NoAssistantAvailable
...
def checkUB(play):
    async def wrapper(_, m: types.Message):
        ...
        if m.chat.type not in (enums.ChatType.GROUP, enums.ChatType.SUPERGROUP):
            await safe_reply("⚠️ This command can only be used in a group.")
            return
        ...
        # /play is intentionally available to all group members.
        if m.chat.id not in db.active_calls:
            try:
                client = await db.get_client(m.chat.id)
            except NoAssistantAvailable:
                await safe_reply(m.lang["no_assistant"])
                return
            try:

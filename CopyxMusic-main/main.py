...
async def _voice_chat_watcher():
    from pyrogram.enums import ChatType
    from handlers.music import stop_playback
    while True:
        for chat_id in list(state.known_groups):
            try:
                call = await call_py.get_call(chat_id)
                active = call is not None
                previous = state.voice_states.get(chat_id, False)
                state.voice_states[chat_id] = active

                if active and not previous:
                    try:
                        await app.send_message(chat_id, "😍 <b>ᴠɪᴅᴇᴏ ᴄʜᴀᴛ sᴛᴀʀᴛᴇᴅ🥳</b>", parse_mode="html")
                    except Exception:
                        pass

                if previous and not active:
                    had_queue = bool(state.chat_queues.get(chat_id))
                    if had_queue:
                ...
                # A transient Telegram/PyTgCalls error must not kill the watcher.
                pass
...
async def main():
    ...
    asyncio.create_task(_voice_chat_watcher())
    await idle()

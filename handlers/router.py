...
from handlers.system import help_command, ping_handler, start_handler
from handlers.voicechat import register_voice_handlers


async def private_play_block(client, message):
    await message.reply_text("⚠️ <b>This command can only be used in a group.</b>", parse_mode="html")


def register_handlers(client):
    register_voice_handlers(client)
    ...
    client.add_handler(MessageHandler(private_play_block, filters.command(["play", "p"]) & filters.private))
